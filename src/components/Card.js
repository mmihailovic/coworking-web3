      ...
      <div className="secondary_container">
        <div className={`${received || expiring ? 'leftpopup' : "left"}`}>
          ...
          <div className={`${received || expiring ? 'date_div_popup' : "date_div"}`}>
            ...
            <p className={`${received || expiring ? 'text2-popup' : "text2"}`}>Expires: {dateParts[0] + "." + dateParts[1] + "." + dateParts[2]}</p>
          ...
          <div className={`${received || expiring ? 'redeem_div_popup' : "redeem_div"}`}>
            ...
            {card.email !== 'Not redeemed' ? <p className={`${received || expiring ? 'text2-popup' : 'text2'}`}>{card.email}</p> : <div id={`${received || expiring ? 'available_div_popup' : "available_div"}`}><p className={`${received || expiring ? 'available_popup' : "available"}`}>Available</p></div>}
